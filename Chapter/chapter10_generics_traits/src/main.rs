...
fn generic_structs() {
    ...
    impl<T: PartialOrd> Pair<T> {
        //泛型加PartialOrd trait,要求类型T必须实现这个trait
...
fn implementing_traits() {
    ...
    impl PartialEq for Rectangle {
        fn eq(&self, other: &Self) -> bool {
            //self表示方法的调用者,Self表示Rectangle一种类型
...
fn orphan_rule() {
    print_example_title("10.8 孤儿规则");

    println!("\n{}", "孤儿规则：".blue().bold());

    println!("只有当trait或类型至少有一个是在当前crate中定义的时候，");
    println!("才能为该类型实现该trait。");

    println!("\n这防止了：");
    println!("1. 两个crate为同一个类型实现同一个trait");
    println!("2. 实现冲突");

    println!("\n可以实现的组合：");
    println!("- 为自定义类型实现标准库trait");
    println!("- 为标准库类型实现自定义trait");
    println!("- 为自定义类型实现自定义trait");

    println!("\n不能实现的组合：");
    println!("- 为标准库类型实现标准库trait");

    pause();
}

fn operator_overloading() {
    print_example_title("10.9 运算符重载");
...
fn associated_types() {
    ...
    impl Iterator for Counter {
        //为类型Counter实现标准库里的Iterator trait
...
fn fully_qualified_syntax() {
    ...
    //调用不同的baby_name方法
    println!("小狗的名字是 {}",Dog::baby_name());
    println!("小动物的名字是 {}",<Dog as Animal>::baby_name());

    // 完全限定语法：<Type as Trait>::function(receiver_if_method, next_arg, ...);
    println!("完全限定语法用于消除歧义");
}

fn supertraits() {
    println!("\n{}", "父trait：".blue().bold());

    trait OutlinePrint:fmt::Display{ //要求类型实现Display trait
        fn outline_print(&self) {
            let output = self.to_string();
            let len=output.len();
            println!("{}", "*".repeat(len + 4)); //repeat把当前字符串重复n次，返回一个新的String
    ...
    println!("父trait要求实现类型必须先实现依赖的trait");
}

fn newtype_pattern(){
    println!("\n{}", "newtype模式：".blue().bold());

    // 为Vec<String>实现Display trait
    ...
    // newtype模式的优点：
    println!("newtype模式的优点：");
    println!("1. 可以为外部类型实现外部trait");
    println!("2. 提供类型安全");
    println!("3. 隐藏内部实现");
    ...
    fn process_user(user_id: UserId) {
        println!("处理用户ID: {}", user_id.0);
    }

    fn process_order(order_id: OrderId) {
        println!("处理订单ID: {}", order_id.0);
    }
